...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text(
        (190, 670),
        f"Powered By: Romeo Abhishek (@julietsupport)",
        (255, 255, 255),
        font=font,
    )
    img.save("final.png")
...
async def play(_, message: Message):
            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "** 😎 I🤞ʌɱ 🥀 Ʀɘɑɗy ♥️ Ƭø ⭐ Play 😎 ...**")
    ...
    if audio:
        ...
        if len(message.command) < 2:
            return await lel.edit(
                "**Usage: /play [Music Name or Youtube Link or Reply to Audio]\n\nIf you face any problem during play song or bot not working in your group properly join our support group**[.](https://telegra.ph/file/0133c912fe21b040a31c5.jpg)"
            )
    ...
    if int(chat_id) in ACTV_CALLS:
        ...
        await message.reply_photo(
            photo="final.png",
            caption="**💥 Added to  Q u e u e  💞 Position » `{}` 🌷 ...**".format(position),
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="**💥 Juliet🤞Music 🎸 Streaming 💞 Song 🔊 On 😍 Voice Chat 🥀 ...**".format(),
           )
...
async def skip(_, message: Message):
    ...
    if int(chat_id) not in ACTV_CALLS:
        await message.reply_text("**💥 Juliet 💞 Nothing 🔇\n🚫 Playing 🌷 ...**")
    else:
