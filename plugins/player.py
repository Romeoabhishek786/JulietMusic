...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text(
        (190, 670),
        f"Powered By: BotsClub (@BotsClubOfficial)",
        (255, 255, 255),
        font=font,
    )
    img.save("final.png")
...
async def play(_, message: Message):
    ...
    lel = await message.reply("**🔎 Searching query please wait ...**")
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                try:
                    ...
                    await lel.edit(
                        "**🤞First 🥀 Ɱake ♥️ Mɘ ⭐ Admin 😎 ...**")
                    return

                try:
                    await USER.join_chat(invitelink)
                    await USER.send_message(
                        message.chat.id, "** 😎 I🤞ʌɱ 🥀 Ʀeady ♥️ Tø ⭐ Play 😎 ...**")

                except UserAlreadyParticipant:
                    pass
                except Exception:
                    await lel.edit(
                        f"**🎸 Ƥɭɘɑsɘ ❤️ Mannualy 🥀 Ʌɗɗ 💫 Ʌssɩsʈɑŋʈ 😔 Øɤ 🎸 Contact ❤️ ʈø : @Romeoabhishek 🥀** ")
    try:
    ...
    if int(chat_id) in ACTV_CALLS:
        ...
        await message.reply_photo(
            photo="final.png",
            caption="**{username}💥 🤞Added 💿 Søɳʛ❗️\n🔊 Ʌʈ 💞 Que » `{}` 🌷 ...**".format(position),
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="🔍__**Video Track Information**__
